  ...
  return (
    <section className={`${styles.flexCenter} md:flex-row flex-col md:space-y-0 space-y-5 pb-12`}>
      <motion.div className={`${styles.flexBetween} flex-col space-y-8 md:items-start md:text-left text-center md:w-[40%] w-[100%]`} variants={slideIn()} initial="hidden" whileInView="show" >
        ...
        <Link href="https://omthecreator.netlify.app/major-project/"><button type='button' className={`text-center px-3 py-2 ${styles.buttonTransition} text-stone-50 dark:text-slate-900 font-normal rounded `}>Case Study</button></Link>
        <Link href="https://avmmotors.com/" target='_blank'><button type='button' className={`text-center px-3 py-2 ${styles.buttonTransition} text-stone-50 dark:text-slate-900 font-normal rounded `}>Live Site</button></Link>
        </div>
