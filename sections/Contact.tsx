...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    emailjs.send('service_by2trvo', 'template_jjegxdr', {
      from_name: form.name,
      to_name: 'Om Patel',
  ...
  return (
    <section
      className= {`${styles.flexBetween} font-poppins py-6 lg:px-0 px-5 xl:mt-12 sm:flex-row flex-col-reverse sm:gap-0 gap-10`}
      id="Contact"
    >
      <motion.form
        ref={formRef}
      ...
      <motion.div className='sm:text-left text-center flex flex-col sm:items-start items-center sm:space-y-5 space-y-10 sm:w-[50%] w-full ' variants={slideIn("right", "tween", 0.2, 1.5)} initial="hidden" whileInView="show" >
        <h1 className=' font-bold text-[36px] w-[80%] dark:text-stone-50 text-slate-900 '>
          Let's{" "}
          <span className=' font-kepler italic text-PurpleTheme '>talk</span> for
          something special
        </h1>
