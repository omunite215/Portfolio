...
const Footer = () => {
  return (
    ...
    >
      ...
      <div
        className="flexCenter md:justify-between md:flex-row flex-col md:space-y-0 space-y-5 md:space-x-96 sm:space-x-56 space-x-0 text-slate-900 dark:text-stone-50"
      >
        <div className={`flexCenter space-x-1 w-fit`}>
          <Image src="/Logo/logo.svg" height={54} width={54} alt="logo" className="animate-spin-slow" />
          <h3 className="flex gap-1 font-poppins font-semibold text-lg">
            Om <span className="sm:block hidden">Patel</span>
          </h3>
        </div>
        <ul
          className="flexCenter md:flex-row flex-col flex-wrap"
        >
          {NavLinks.map((items) => (
            <li
              key={items.label}
